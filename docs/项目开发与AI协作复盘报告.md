...
| **Electron IPC & ArrayBuffer**<br>*(打破桌面端沙盒与依赖)* | **挑战**：本地上传的 Excel 路径在 Electron 渲染进程中被沙盒化，且原系统重度依赖外部 Python 脚本，打包体积庞大。<br>**解决**：AI 敏锐切入 Node `xlsx` 库替换底层 Python 脚本，并巧妙使用 `ArrayBuffer` 作为 IPC 的通信载体，完美绕开本地文件对象限制。   | 🌟🌟🌟🌟🌟<br>**极高**。AI 不仅仅写代码，还主导了彻底“去 Python 化”的架构重构，极大提升了本地应用的内聚性和渲染性能。     |
| **Multi-role AI Reporting**<br>*(多角色结构化推演)*        | **挑战**：不同财务人员解读数据的视角差别巨大，原有的单一提示词缺乏张力；又需保持良好的高阶提示词自定义体验。<br>**解决**：AI 结合产品思维（Product Insight），自动延伸出“审计/投资/风控/管理”四大矩阵角色映射，并辅以极其顺滑的 UI 下拉与抽屉定制交互。 | 🌟🌟🌟🌟🌟<br>**极高**。属于“产品+工程”的双轨爆发。AI 补齐了产品经理的细分用户画像洞察，并在 TDD 的约束下完成状态机联调。 |
...
3. **TDD (测试驱动开发) 的完美搭档**：
   - 在修 OCR Bug 以及在 v1.2.2 修复大模型接口强规范限制（如对 `temperature` 下限的探测）时，AI 表现出极好的工程素养：倾向于**先写测试用例**（模拟脏数据），**再改核心代码**。这种全量回归验证模式极大增强了重构代码时的信心，避免了“修一个 Bug 引入两个新 Bug”的惨剧。
4. **性能“深潜分析”与架构重构驱动 (v1.2.1 专项)**：
   - **高光回顾**：面对 Gemini API 响应缓慢的问题，AI 没有盲目盲目修改，而是通过添加毫秒级精准日志，锁定了输出 Token 冗余这一根本矛盾。
   - **决策透明**：AI 提出了“弃 JSON 换 Markdown”的重构建议，在不改变业务精度的前提下，将响应报文体积压缩 70% 以上，并同步主导本地高性能 Markdown 解析器的开发，展现了极强的架构重塑能力。
5. **代理网关兼容性“避坑指南” (v1.2.2 专项)**：
   - **解决痛点**：跨供应商（如从 Gemini 切换至部分国产智谱代理）时的异常 400 校验。
   - **洞察**：AI 准确嗅探出部分国产模型库不允许 `temperature` 设为绝对的 `0`，以及 Node.js 的 LangChain 特定打色机制中对于 `openAIApiKey` 泛型映射漏洞，仅凭报错极速通过修改 `0.01` 及防御性入参双核验证消散问题。
