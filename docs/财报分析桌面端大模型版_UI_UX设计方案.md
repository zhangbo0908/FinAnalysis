...
**用户目标：** 将三大表数据投喂大模型，结合用户所属身份（审计/投资/风控/管理层）与特定提示词，获取极具倾向性的深度决策参考。
- **布局形态**：左右分屏。左侧为**数据源概览与大模型推演基座配置区**（包含数据卡片、拖拽上传区、角色配置区），右侧为 Markdown 流式分析渲染区。
- **数据源“双链路”极简交互**：
  - **内联缓存流转**：从“校对台”直接传来的 JSON 数据，左侧直接呈现为平铺的三张状态卡片（显示行数并标记 ✅ 就绪）。
  - **本地 Excel 快车道（空状态下）**：通过“拖拽区域”直接上传标准版三表 Excel 以重组状态卡片。
- **多角色矩阵式推演操控 (核心更新)**：
  - 在操作区提供一个符合产品洞察的 **「分析视阈 Selector」**，内置 4 种原厂调优角色选项：`👔 中立审计与财务总监`、`📈 二级市场价值投资`、`💼 企业内部经营剖析`、`🛡️ 信贷风控雷区排查`。
  - **无缝化交互**：改变 Selector 将隐式切换主进程请求使用的分析提示词侧重点。选择操作无需刷新页面或繁琐确认，直接在下次 "开始分析" 时生效。
- **高阶定制分析模式 (渐进式披露原则)**：
  - 在主按钮组合下方，配置带 `Ghost` 风格的轻量级操作入口：“⚙️ 原生指令高阶设定”。
  - 展开的抽屉 (Drawer) / 全屏弹窗不仅是一个空框，而是会**响应并呈现当前 Select 角色的底层完整系统 Prompt 文字**！
  - 用户可在这个大文本域里针对所选角色继续做个人偏好上的补充指令，如："在前述的投资护城河分析上，务必加上折现率 10% 时的 DCF 试算模型..."。面板内提供保存与重置键。
- **四态状态机 UI**：
  - **空态**：呈现拖拽上传卡片与未就绪提示。
  - **就绪态**：展示数据就绪卡片，底部高亮"开始分析 ✨"按钮。
