...
### 4.3 报告生成重塑 (Agentic Reporting)
用户导入了自己撰写的 Markdown 模板（如：“请计算本期流动资产同比增加率并给出一段 50 字评价”）。这在过去靠晦涩难懂的业务计算引擎去 `eval` 公式，现在直接作为 User Prompt 发送给底层大模型（LLM），附随财报 JSON 上下文，由 LLM 完成高度智能化的报表撰写并原样输出 Markdown 到前端编辑器。
