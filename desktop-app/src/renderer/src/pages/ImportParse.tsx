  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-6">
      ...
      <p className="text-muted-foreground">
        将您的财务报表 PDF 拖入下方虚线框进行纯本地图片层析提取。切分后的高清图片流将发往 VLM
        模型抽取结构化财报大表。
      </p>

      {!result && <Dropzone onFileAccepted={handleFileAccepted} isProcessing={isProcessing} />}
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex justify-between items-center mb-2">
            <div className="space-y-1">
              <h2 className="text-xl font-semibold">切片成功 (共 {result.totalPages} 页)</h2>
              <p className="text-sm text-muted-foreground hidden sm:block">
                请勾选目标财报三大表所在页，剔除无关注脚和封面可极大加速大模型处理速度。
              </p>
            </div>
          ...
          <div className="flex justify-between items-center pt-4 sticky bottom-4 bg-background/90 backdrop-blur-sm p-4 rounded-lg shadow-lg border">
            ...
            <Button
              ...
              onClick={() => {
                const selectedImagesBase64 = result.imagesBase64.filter((_, i) =>
                  selectedPages.has(i)
                )
