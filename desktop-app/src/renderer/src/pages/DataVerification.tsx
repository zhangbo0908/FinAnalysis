...
function generateColumnsFromData(data: Record<string, any>[]) {
  ...
  return keys.map((key) => {
    ...
    const isSubjectColumn =
      key.includes('项目') || key.includes('资产') || key.includes('负债') || key.includes('权益')
...
export function DataVerification() {
  ...
  const handleExtraction = async () => {
    ...
    try {
      ...
      if (!currentSettings) {
        throw new Error(
          `系统选定的服务商 '${activeProvider}' 尚未配置 API Key。请前往设置页填写并保存。`
        )
      }
      ...
      setError(
        err.message || '通过大模型提取财务报表失败。请检查配置页的网络代理与 API Key 是否正确有效。'
      )
    } finally {
        ...
        <div className="p-4 border-b flex justify-between items-center bg-card">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              disabled={isExtracting || balanceSheet.length === 0}
              className="gap-2"
            >
          ...
          {isExtracting ? (
            <div className="absolute inset-0 z-10 bg-background/50 backdrop-blur-sm flex flex-col items-center justify-center space-y-4">
              ...
              <p className="font-semibold text-lg text-primary animate-pulse w-72 text-center">
                正在并发处理 {imagesBase64.length} 张图片
                <br />
                请耐心等待 {providerName} 思考...
              </p>
              {warningMsg && (
